...
function startOverlay(box_id, image, rarity, new_item) {
  ...
  document.getElementById("fake_box").innerHTML =
    `<div id="fake_box_div" style="position: fixed; width: ${rect.width}px; height: ${rect.height}px; top: ${rect.top}px; left: ${rect.left}px; transition: left 1.5s ease;"><img src=${image} style="padding: 30px;"></img></div>`;
  document.getElementById("box_overlay").style.zIndex = "10";
  document.getElementById("box_overlay").style.backgroundColor =
    "rgba(60, 163, 238, 1)";
  ...
  setTimeout(function () {
    document.getElementById("fake_box_div").style.left =
      `calc(50% - ${rect.width / 2}px)`;
  }, 1500);
  setTimeout(function () {
  ...
  setTimeout(function () {
    ...
    document
      .getElementById("spline_box")
      .removeChild(document.getElementById("spline_box").firstChild);
    reloadSpine();
  }, 13000);
}
...
export default function BasicTabs({
  ...
  useEffect(() => {
    async function saveAvatar() {
      try {
        if (username != "Loading") {
          await postData("api/users/updateUnlocked", {
            username,
            unlockedAvatars,
            unlockedBorders,
            unlockedBackgrounds,
  ...
  function clickBox(cost, id, spend, image) {
    if (coins >= cost) {
      ...
      while (new_item === null && tries < 10) {
        ...
        if (num === 0) {
          const unlockables = avatarImageList.filter(
            (image) => !unlockedAvatars.includes(image.name),
          );
          ...
          const unlockables = borderImageList.filter(
            (image) => !unlockedBorders.includes(image.name),
          );
          if (unlockables.length !== 0) {
            const num2 = getRandomInt(0, unlockables.length);
            rarity = "rare";
          ...
          const unlockables = backgroundImageList.filter(
            (image) => !unlockedBackgrounds.includes(image.name),
          );
          if (unlockables.length !== 0) {
            const num2 = getRandomInt(0, unlockables.length);
            rarity = "epic";
            new_item = unlockables[num2];
            setUnlockBackground((prev) =>
              prev.concat([unlockables[num2].name]),
            );
            new_item = `<img src=${new_item.source} style=" max-width: none; max-height: none; position: absolute; top: 50%; left: 50%; transform: translate(-48%, -50%); width: 300px; border-radius: 20px;"></img>`;
          }
        }
  ...
  function LootBox({ id, name, image, cost }) {
    return (
      <Grid
        ...
        onClick={() => clickBox(cost, id, spendCoins, image)}
      >
        <p
          className={
            coins >= cost
              ? "russo-one-regular text-3xl text-white"
              : "russo-one-regular text-3xl text-red-500"
          }
        >
          {name}
        </p>
        ...
        <p
          className={
            coins >= cost
              ? "russo-one-regular text-4xl text-white"
              : "russo-one-regular text-4xl text-red-500"
          }
        >
          {cost}🪙
        </p>
  ...
  return (
    <Box sx={{ width: "100%" }}>
      ...
      <CustomTabPanel
        value={value}
        index={1}
        style={{ height: "60vh", overflowY: "scroll", overflow: "auto" }}
      >
        <Avatars
      ...
      >
        <Borders
          unlocked={unlockedBorders}
          change={setBorder}
          current={currentBorder}
        />
      </CustomTabPanel>
    </Box>
  );
