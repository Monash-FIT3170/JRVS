  ...
  return (
    <Grid container spacing={2} columns={25}>
        {Object.keys(imageList).map((imageName, index) => (
            ...
            <div style={{ border: '2px solid #2196f3', backgroundColor: 'white', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', borderRadius: '20px', boxShadow: '0px 4px 8px rgba(0, 0, 0, 0.1)', backgroundColor: "#80BAE4", height: '100%', width: '100%' }} onClick={() => handleImageClick(imageName)}>
                <img src={imageList[imageName]} alt={imageName} style={{padding: '20px'}} />
                <div id={'avatar_'+imageName} style={{width: '100%', position: 'relative'}}>
                    {imageName == selected ? <img src={Tick} style={{border: '2px solid #2196f3', zIndex: '1', padding: '20px', position: 'absolute', bottom: '0', right: '0', width:'100%', backgroundColor: 'rgba(0,0,0,0.5)', borderRadius: '20px'}}/> : null}
                </div>
            </div>
