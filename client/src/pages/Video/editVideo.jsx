...
const EditVideo = () => {
  ...
  const validateAndFormatUrl = (url) => {
    ...
    if (watchUrlPattern.test(url)) {
      ...
      setError('Please enter a valid YouTube URL. Example: https://www.youtube.com/watch?v=sQw6y3fh0 or https://www.youtube.com/embed/sQw6y3fh0');
      return null;
    }
  };
  ...
  return (
    <Box sx={{ width: '100vw', height: '100vh', backgroundColor: '#3CA3EE', overflow: 'auto' }}>
      <MenuBar />
      <Box sx={{ position: 'relative', width: '100vw', backgroundColor: '#3CA3EE', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
        <Box sx={{ backgroundColor: 'white', padding: '20px', borderRadius: '8px', boxShadow: '0 4px 8px rgba(0, 0, 0, 0.2)', display: 'flex', flexDirection: 'column', alignItems: 'center', width: '100%', maxWidth: '600px' }}>
          ...
          <form onSubmit={saveEdit} style={{ display: 'flex', flexDirection: 'column', width: '100%' }}>
            <TextField
              ...
              required
              sx={{ marginBottom: '15px', width: '100%', backgroundColor: 'white' }}
            />
            <TextField
              label="URL"
              value={currentUrl}
            ...
            <TextField
              ...
              multiline
              rows={2}
              sx={{ marginBottom: '5px', width: '100%', backgroundColor: 'white' }}
            />
            {error && (
            ...
            <Button
              type="submit"
              variant="contained"
              sx={{ marginTop: '20px', backgroundColor: '#FFC93C', ':hover': { backgroundColor: '#2196F3' } }}
            >
              Save Video
            </Button>
          </form>
